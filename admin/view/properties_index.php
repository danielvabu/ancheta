...
<div>
<h4><?php echo __("Properties"); ?> <span class="badge badge-primary"><?php if(isset($sale->_numOfRows)){echo $sale->_numOfRows;}else{echo "0";} ?></span></h4>

<nav class="navbar navbar-default" role="navigation">
<div class="navbar-header">

<form action="<?php echo PATO; ?>properties/filtrar/" method="post" name="properties-filtrar" id="properties-filtrar" class="form-inline navbar-text">
...
if($msg==1)echo __("No hay Properties");
if($msg==2)echo __("Para ver resultados por favor filtre primero");
?></div></div><?php }else{ ?>
<div class="table-responsive"><table border="0" id="tb_sale_properties" class="table table-bordered table-striped table-hover table-condensed">
<thead>
	<tr class="hidden">
...
<a href="<?php echo PATO; ?>properties/ver/<?php echo $sale->fields["id"]; ?>/" data-toggle="tooltip" title="<?php echo __("Ver"); ?>" class="btn btn-default"><i class="fas fa-eye"></i></a>

<a href="<?php echo PATO; ?>properties/editar/<?php echo $sale->fields["id"]; ?>/" data-toggle="tooltip" title="<?php echo __("Editar"); ?>" class="btn btn-default"><i class="fas fa-edit"></i></a>
...
$(function(){
	...
	$("#tb_sale_properties").tablesorter({headers: {0: {sorter: false}}, widgets: ["zebra"]});
	$("#tb_sale_properties").tablesorterPager({container:$(".pager2"),ajaxUrl:null,output:"{startRow} <?php echo __("a"); ?> {endRow} <?php echo __("de un total de"); ?> {totalRows}",updateArrows:true,page:0,size:<?php echo RPP; ?>,fixedHeight:false,removeRows:true,cssNext:".next",cssPrev:".prev",cssFirst:".first",cssLast:".last",cssPageDisplay:".pagedisplay",cssPageSize:".pagesize",cssDisabled:"disabled"});
});
function eliminarproperties(a){
	confirma1('<?php echo iinterro().__("Esta seguro de eliminar Properti?"); ?>','<?php echo __("Si"); ?>','eliminandoproperties',a)}
function eliminandoproperties(a){window.location.href = "<?php echo PATO; ?>properties/eliminar/"+a+"/";}
</script><?php } ?>

<form class="form-inline">

<div class="form-group"><button type="button" class="btn btn-primary form-control" onClick="location.href='<?php echo PATO; ?>properties/agregar/';"><i class="fas fa-plus"></i>&nbsp;&nbsp;<?php echo __("Nuevo"); ?></button></div>
