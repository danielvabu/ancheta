...
<h4><?php echo __("Nuevo Anchet"); ?></h4>

<form id="ancheta-agregando" name="ancheta-agregando" class="form-horizontal" method="post" action="<?php echo PATO; ?>ancheta/agregando/" enctype="multipart/form-data">
...
<label for="sessione_id" class="col-2"><?php echo __("Sessione"); ?></label>
<div class="col-7"><select name="sessione_id" id="sessione_id" class="custom-select" data-placement="right" data-original-title="<?php echo __("Dato requerido"); ?>" data-content="<?php echo __("Por favor seleccione el Sessione"); ?>" required>
		<option value="0" selected="selected"><?php echo __("Seleccione Sessione"); ?></option><?php
if(!$sessiones->EOF){
	while(!$sessiones->EOF){ ?>
    	<option value="<?php echo $sessiones->fields["id"]; ?>"><?php echo $sessiones->fields["nombre"]; ?></option><?php
		$sessiones->MoveNext();
	}
	$sessiones->Move(0);
} ?></select></div>
...
<div class="col-7"><select name="cantidad" id="cantidad" class="custom-select" data-placement="right" data-original-title="<?php echo __("Dato requerido"); ?>" data-content="<?php echo __("Por favor seleccione la Cantidad"); ?>" required>
			<option value="0" selected="selected"><?php echo __("Seleccione Cantidad"); ?></option>
					</select></div>
</div>
...
$(function(){
	...
	$("#ancheta-agregando").submit(function(){
